...
module ultrasonic_datapath(
    input               clk,
    input               rst,
    input               i_echo,
    input               i_control_unit_start,
    output              o_trig,
    output [31:0]       o_fnd_ultrasonic_data
);
    wire tick_us, w_btn;
    wire [8:0] w_dist;

    tick_gen #(.CLOCK_CYCLE_1SEC(100000000), .CLOCK_NS(10), .TARGET_TIME(1000)) 
        U_TICK_GET (
            .clk(clk),
            .rst(rst),
            .b_tick(tick_us)
    ...
    sr04_ctrl U_SR04_CTRL (
        ...
        .rst(rst),
        .tick_us(tick_us),
        .start(i_control_unit_start),
        .echo(i_echo),
        .dist(w_dist),
        .trigger(o_trig)
    );

    assign o_fnd_ultrasonic_data = {22'b0, w_dist};
...
module tick_gen #(
    ...
    localparam TICK_CYCLES = TARGET_TIME / CLOCK_NS;
